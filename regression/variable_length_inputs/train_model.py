...
def train_model(logfc_train_set, logfc_test_set, train_set, test_set, inference_func, params, logdir=None):
    ...
    # do inference step
    inference_results = inference_func(x_image, y, keep_prob, params)
    train_step, loss, prediction, var_dict, train_step_logfc, loss_logfc, prediction_logfc = inference_results
    ...
    if params['TRAIN_KD']:
        ...
        train_set_temp['keep'] = [int(np.random.random() > 0.75) if x == 'grey' else 1 for x in train_set_temp['color']]
    ...
    while epoch_counter < params['MAX_EPOCH']:
        ...
        if params['TRAIN_KD']:
            ...
            train_labels = train_group[['log_kd']].values
            train_colors = list(train_group['color'].values)
            train_features = np.zeros((len(train_colors), 16 * params['MIRLEN'] * params['SEQLEN']))
            for i, row in enumerate(train_group.iterrows()):
                mirseq = row[1]['mirseq'][-params['MIRLEN']:]
                train_features[i, :] = helpers.make_square(mirseq, row[1]['seq']).flatten()

            # train step
            _ = sess.run(train_step, feed_dict={x: train_features,
        ...
        if params['TRAIN_LOGFC']:
            ...
            logfc_train_set_batch_labels = logfc_train_set_group[['logFC']].values
            logfc_train_set_batch_colors = list(logfc_train_set_group['color'].values)
            logfc_train_set_batch_features = np.zeros((len(logfc_train_set_batch_colors), 16 * params['MIRLEN'] * params['SEQLEN']))

            for i, row in enumerate(logfc_train_set_group.iterrows()):
                mirseq = row[1]['mirseq'][-params['MIRLEN']:]
                logfc_train_set_batch_features[i, :] = helpers.make_square(mirseq, row[1]['seq']).flatten()

            # train step
            _ = sess.run(train_step_logfc, feed_dict={x: logfc_train_set_batch_features,
                                                      keep_prob: params['KEEP_PROB_TRAIN'],
                                                      y: logfc_train_set_batch_labels})
        ...
        if batch_counter%params['REPORT_INT'] == 0:
            ...
            logfc_test_loss, logfc_test_set_prediction = sess.run([loss_logfc, prediction_logfc],
            ...
            if logdir is not None:
                ...
                if params['TRAIN_KD']:
                    train_loss, train_prediction = sess.run([loss, prediction], feed_dict={x: train_features,
                                                                                           keep_prob: params['KEEP_PROB_TRAIN'],
                                                                                           y: train_labels})

                    helpers.graph_predicted_v_actual(params['NCOLS'], params['OUT_NODES'], train_prediction, train_labels, 
                                                train_colors, os.path.join(logdir, 'scatter_train.pdf'),
                                                log_scale=params['LOG_SCALE'])

                if params['TRAIN_LOGFC']:
                    logfc_train_loss, logfc_train_batch_prediction = sess.run([loss_logfc, prediction_logfc],
                                                                              feed_dict={x: logfc_train_set_batch_features,
                                                                                         keep_prob: 1.0,
                                                                                         y: logfc_train_set_batch_labels})

                    helpers.graph_predicted_v_actual(params['NCOLS'], params['OUT_NODES'], logfc_train_batch_prediction,
                                                    np.repeat(logfc_train_set_batch_labels, params['OUT_NODES'], axis=1), 
                                                    logfc_train_set_batch_colors, os.path.join(logdir, 'scatter_train_logfc.pdf'),
                                                    log_scale=params['LOG_SCALE'])
