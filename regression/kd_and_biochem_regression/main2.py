...
if __name__ == '__main__':
    ...
    with tf.Session(config=tf.ConfigProto(intra_op_parallelism_threads=24)) as sess:
        ...
        kd_mask = tf.placeholder(tf.float32, shape=[None, 1], name='kd_mask')
        ...
        with tf.name_scope('layer1'):
            ...
            layer1_kd = tf.nn.relu(preactivate1_kd_bn)
        ...
        with tf.name_scope('layer2'):
            ...
            layer2_kd = tf.nn.relu(preactivate2_kd_bn)
        ...
        with tf.name_scope('layer3'):
            ...
            layer3_kd = tf.nn.relu(preactivate3_kd_bn)
        ...
        with tf.name_scope('final_layer'):
            ...
            pred_kd_ind = tf.matmul(layer_flat_kd, w4) + b4
            pred_kd = pred_kd_ind[-1 * BATCH_SIZE_KD:, :1]
            pred_kd_tpm_flat = pred_kd_ind[:-1 * BATCH_SIZE_KD, :1]
            pred_kd_tpm = tf.reshape(pred_kd_tpm_flat, [NUM_TRAIN, -1])

        pred_nbound = tf.reduce_sum(tf.multiply(tf.nn.sigmoid(freeAGO + pred_kd_tpm), tpm_mask), axis=1)
        ...
        for epoch_counter in range(NUM_EPOCHS):
            ...
            for gene_ix, tpm_row in enumerate(train_tpm.iterrows()):
                ...
                if (gene_ix % REPORT_INT) == 0:
                    ...
                    train_tpm_preds = sess.run(pred_tpm, feed_dict=feed_dict)
                    ...
                    for seq_list in test_seqs:
                        ...
                        batch_tpm_kd_x = np.array(batch_tpm_kd_x).reshape(len(seq_list), 4*MIRLEN,4*SEQLEN, 1)
