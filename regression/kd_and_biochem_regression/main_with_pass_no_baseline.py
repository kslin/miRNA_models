...
if __name__ == '__main__':
    ...
    MIRLEN = 12
    SEQLEN = 12
    BATCH_SIZE_BIOCHEM = 75
    BATCH_SIZE_REPRESSION = 5
    KEEP_PROB_TRAIN = 0.5
    STARTING_LEARNING_RATE = 0.004
    LAMBDA = 0.001
    NUM_EPOCHS = 10
    REPORT_INT = 50
    ...
    # create data object
    biochem_train_data = objects.BiochemData(data, cutoff=0.95)
    ...
    subset = np.random.choice(np.arange(len(test_tpm)), size=100)
    ...
    test_mirseq_one_hot = ONE_HOT_DICT[options.TEST_MIRNA]
    current_ix = 0
    for seq_list in test_seqs:
        ...
        if len(seq_list) == 0:
            ...
            for seq in seq_list:
                temp = np.outer(test_mirseq_one_hot, helpers.one_hot_encode_nt_new(seq, SEQ_NTS))
    ...
    for seq_list in test_seqs:
        ...
        if len(seq_list) == 0:
            ...
            for seq in seq_list:
                temp = np.outer(test_mirseq_one_hot, helpers.one_hot_encode_nt_new(seq, SEQ_NTS))
                test_combined_x_pass[current_ix, :, :] = temp - 0.25
                current_ix += 1
    ...
    with tf.Session(config=tf.ConfigProto(intra_op_parallelism_threads=24)) as sess:
        ...
        with tf.name_scope('final_layer'):
            ...
            # apply final layer
            _pred_ind_values = ((tf.matmul(_layer_flat, _w4) + _b4) * NORM_RATIO) - ZERO_OFFSET
        ...
        if options.DO_TRAINING:
            ...
            _pred_repression_flat = _pred_ind_values[:-1 * BATCH_SIZE_BIOCHEM, :]
            ...
            _intercept_fit = tf.reshape((tf.reduce_mean(_repression_y, axis=1) - tf.reduce_mean(_pred_logfc, axis=1)), [-1, 1])
            _pred_tpm = _pred_logfc + _intercept
            _pred_tpm_fit = _pred_logfc + _intercept_fit
            ...
            with tf.control_dependencies(_update_ops):
                ...
                _train_step_fit = tf.train.AdamOptimizer(STARTING_LEARNING_RATE).minimize(_loss_fit)
        ...
        if options.DO_TRAINING:
            ...
            while True:
                ...
                if next_epoch:
                    ...
                    pred_ind_nbound_test = 1.0 / (1.0 + np.exp((-1.0 * pred_ind_values_test.flatten()) - current_freeAGO))
                    pred_nbound_test = []
                    ...
                    pred_ind_values_test = sess.run(_pred_ind_values, feed_dict=feed_dict)
                    pred_ind_nbound_test = 1.0 / (1.0 + np.exp((-1.0 * pred_ind_values_test.flatten()) - (current_freeAGO - test_freeAGO_diff)))
                    pred_nbound_test = []
                    prev = test_seq_utr_boundaries[0]
                    for bound in test_seq_utr_boundaries[1:]:
                        pred_nbound_test.append(np.sum(pred_ind_nbound_test[prev:bound]))
                    ...
                    if last_batch:
                        ...
                        freeAGO_df = pd.DataFrame({'mir': train_mirs,
                                                   'guide': trained_freeAGO_site,
                                                   'passenger': trained_freeAGO_pass})
