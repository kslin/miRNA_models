        ...
        with open(os.path.join(options.OUTDIR, 'orf_sites.txt'), 'a') as orf_sites_outfile:
            for ix, row in enumerate(TPM.iterrows()):
                ...
                for mir in ALL_MIRS:
                    ...
                    seqs, locs = get_site_features.get_sites_from_utr(orf, site8, overlap_dist=options.OVERLAP_DIST, only_canon=True)
                ...
                if len(feature_df) > 0:
                    ...
                    feature_df.to_csv(outfile, sep='\t', header=False, index=False, float_format='%.3f')
