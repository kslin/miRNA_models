...
if __name__ == '__main__':
    ...
    parser.add_option("--rnaplfold_bg", dest="RNAPLFOLD_BG", help="SA background file")
    ...
    parser.add_option("--write_seqs", dest="WRITE_SEQS", help="write seqs found")
    ...
    SA_bg = pd.read_csv(options.RNAPLFOLD_BG, sep='\t', index_col='seq')
    SA_BG_DEFAULT = 0.412451

    # read in orf sequences
    ORF_SEQS = pd.read_csv(options.ORF_FILE, sep='\t', header=None, index_col=0)

    with open(options.WRITE_SEQS, 'a') as plain_writer:
        with tf.python_io.TFRecordWriter(options.OUTFILE) as tfwriter:
            for ix, row in enumerate(TPM.iterrows()):
                ...
                for mir in ALL_MIRS:
                    ...
                    seqs, locs = get_site_features.get_sites_from_utr(transcript_seq, site8,
                        overlap_dist=options.OVERLAP_DIST, only_canon=options.ONLY_CANON)

                    stypes = [utils.get_centered_stype(site8, seq) for seq in seqs]

                    keep_sites = []
                    for (seq, loc, stype) in zip(seqs, locs, stypes):
                        if loc < orf_len:
                            if stype != 'no site':
                                keep_sites.append([seq, loc, stype])
                        else:
                            keep_sites.append([seq, loc, stype])

                    nsites = len(keep_sites)
                    if nsites > 0:
                        keep_seqs = [x[0] for x in keep_sites]
                        keep_locs = [x[1] for x in keep_sites]
                        keep_stypes = [x[2] for x in keep_sites]

                        pct_df_temp = pct_df[pct_df['ID'] == transcript + mir]
                        if len(pct_df_temp) == 0:
                            pct_df_temp = None
                        features = get_site_features.get_ts7_features(mirseq, keep_locs, keep_stypes, transcript_seq, utr_len, orf_len,
                            options.UPSTREAM_LIMIT, rnaplfold_data, pct_df_temp)

                        feature_df = pd.DataFrame(features, columns=['in_ORF','logSA','threep','PCT','localAU','min_dist','utr3_length','orf_length'])
                        feature_df['transcript'] = transcript
                        feature_df['mir'] = mir
                        feature_df['seq'] = keep_seqs
                        feature_df['loc'] = keep_locs
                        feature_df['stype'] = keep_stypes
                        feature_df['logSA_bg'] = SA_bg.query('mir == @mir').reindex(keep_seqs)['logp'].values
                        feature_df['logSA_diff'] = feature_df['logSA'] - feature_df['logSA_bg']
                        feature_df['logSA_diff'] = feature_df['logSA_diff'].fillna(SA_BG_DEFAULT)

                        feature_df.to_csv(plain_writer, sep='\t', index=False, float_format='%.3f')

                        long_seq = ''.join(keep_seqs)

                        keep_cols = ['in_ORF','logSA_diff','threep','PCT','localAU','min_dist','utr3_length','orf_length']
                        feature_dict['{}_seqs_1hot'.format(mir)] = utils._float_feature(utils.one_hot_encode(long_seq))
                        feature_dict['{}_ts7_features'.format(mir)] = utils._float_feature(list(feature_df[keep_cols].values.flatten()))
