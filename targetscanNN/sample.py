...
if __name__ == '__main__':
    ...
    with tf.Session() as sess:
        ...
        # convolution layer for each 4x4 box representing each dinucleotide
        in_channels = out_channels
        out_channels = 4
        layer2 = helpers.make_convolution_layer(layer1, 4, 4, in_channels, out_channels,
                                                  1, 1, 'conv6x6', padding='SAME', act=tf.nn.relu)

        in_channels = out_channels
        out_channels = 8
        layer2_1 = helpers.make_convolution_layer(layer2, 4, 4, in_channels, out_channels,
                                                  1, 1, 'conv3x3', padding='VALID', act=tf.nn.relu)
        ...
        # in_channels = out_channels
        # out_channels = 64
        # layer2_1_pre = helpers.make_convolution_layer(layer2, 4, 4, in_channels, out_channels,
        ...
        layer_flat_dim = layer2_1.get_shape().as_list()
        print(layer_flat_dim)
        layer_flat_dim = layer_flat_dim[1] * layer_flat_dim[2] * layer_flat_dim[3]
        print(layer_flat_dim)
        # layer_flat_dim = out_channels * 62 * 40
        # layer_flat_dim = out_channels * 23 * 40
        layer_flat = tf.reshape(layer2_1, [-1, layer_flat_dim])
        ...
        train_step, accuracy = helpers.make_train_step('classification', layer4, y)
        ...
        train, test = sequence_helpers.generate_sample_data(10, options.DATALEN, options.MIRLEN, options.SEQLEN)
        ...
        for i in range(1, num_epoch+1):
            ...
            if i%report_int == 0:
                ...
                acc, summary = sess.run([accuracy, merged], feed_dict={x: test.features,
                                                                       ...
                                                                       keep_prob: 1.0,
                                                                       y: test.labels})
                # predicted = sess.run(layer4, feed_dict={x: test.features,
                #                                                        extra: test.extra_features,
                #                                                        keep_prob: 1.0,
                #                                                        y: test.labels})
